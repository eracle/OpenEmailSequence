import json
from typing import Any, Callable, Dict, List, Optional, Set, Tuple, Union

from django import forms
from django.conf import settings
from django.contrib import admin
from django.core.handlers.wsgi import WSGIRequest
from django.http import HttpRequest, HttpResponse
from django.shortcuts import get_object_or_404, render
from django.urls import URLPattern, path

from email_sequences.campaigns.admin import CampaignAdmin
from email_sequences.sequences import configured_message_classes, message_class_for
from email_sequences.models import Campaign, Sequence, QuerySetRule, SentEmail
from email_sequences.utils import get_simple_fields, get_user_model

User = get_user_model()


class QuerySetRuleInline(admin.TabularInline):
    model = QuerySetRule


class SequenceForm(forms.ModelForm):
    message_class = forms.ChoiceField(
        choices=((k, "{k} ({v})".format(k=k, v=v)) for k, v in configured_message_classes().items()),
    )

    class Meta:
        model = Sequence
        exclude: List[str] = []

    def __init__(self, *args, **kwargs):
        """
        Overriding init form to change help text in body_html_template field according to SEQUENCE_UNSUBSCRIBE_USERS
        """
        super(SequenceForm, self).__init__(*args, **kwargs)
        unsubscribe_users = getattr(
            settings,
            "SEQUENCE_UNSUBSCRIBE_USERS",
            False,
        )
        if unsubscribe_users:
            basic_help_text = self.fields["body_html_template"].help_text
            unsubscribe_help_text = """
            <br/><br/>
            <b>UNSUBSCRIBE USERS:</b>
            <br/>
            <b>Sequence:</b> If you wish to give the user an unsubscribe link to this Sequence, please include
            <b>'http://yourdomain.com{{unsubscribe_link_sequence}}'</b> wherever you want in the email.
            <br/>
            <b>Campaign:</b> If you wish to give the user an unsubscribe link to a Campaign
            (it will only work if you have a campaign related to this Sequence), please include
            <b>'http://yourdomain.com{{unsubscribe_link_campaign}}'</b> wherever you want in the email.
            <br/>
            <b>General:</b> If you wish to give the user an unsubscribe link to ALL emails send by Sequence library
            please include <b>'http://yourdomain.com{{unsubscribe_link}}'</b> wherever you want in the email.
            <br/>
            <br/>
            Replace the HTTP protocol and yourdomain.com with the appropriate ones.
            """
            self.fields["body_html_template"].help_text = f"{basic_help_text}{unsubscribe_help_text}"


class SequenceAdmin(admin.ModelAdmin):
    list_display = ("name", "enabled", "message_class")
    inlines = [
        QuerySetRuleInline,
    ]
    form = SequenceForm
    users_fields: Union[str, List[str]] = []

    def av(self, view: Callable) -> Callable:
        return self.admin_site.admin_view(view)

    def timeline(
        self,
        request: WSGIRequest,
        sequence_id: int,
        into_past: int,
        into_future: int,
    ) -> HttpResponse:
        """
        Return a list of people who should get emails.
        """

        sequence = get_object_or_404(Sequence, id=sequence_id)

        shifted_sequences = []
        seen_users: Set[int] = set()
        for shifted_sequence in sequence.sequence.walk(into_past=int(into_past), into_future=int(into_future) + 1):
            shifted_sequence.prune()
            shifted_sequences.append(
                {
                    "sequence": shifted_sequence,
                    "qs": shifted_sequence.get_queryset().exclude(
                        id__in=seen_users,
                    ),
                },
            )
            if not sequence.can_resend_sequence:
                seen_users.update(shifted_sequence.get_queryset().values_list("id", flat=True))

        return render(request, "email_sequences/sequence_timeline.html", locals())

    def get_mime_html_from_alternatives(self, alternatives: List[Tuple[str, str]]) -> Tuple[str, str]:
        html = ""
        mime = ""
        for body, mime in alternatives:
            if mime == "text/html":
                html = body
                mime = "text/html"
        return html, mime

    # Ignoring this line because mypy says User is not a valid type
    def get_mime_html(self, sequence: Sequence, user: User) -> Tuple[str, str]:  # type: ignore
        # Ignoring this line because mypy says SequenceMessage is not callable
        sequence_message = message_class_for(  # type: ignore
            sequence.message_class,
        )(sequence.sequence, user)
        if sequence_message.message.alternatives:
            return self.get_mime_html_from_alternatives(sequence_message.message.alternatives)
        html = sequence_message.message.body
        mime = "text/plain"
        return html, mime

    def view_sequence_email(
        self,
        request: WSGIRequest,
        sequence_id: int,
        into_past: int,
        into_future: int,
        user_id: int,
    ) -> HttpResponse:
        sequence = get_object_or_404(Sequence, id=sequence_id)
        user = get_object_or_404(User, id=user_id)

        html, mime = self.get_mime_html(sequence, user)

        return HttpResponse(html, content_type=mime)

    def build_extra_context(self, extra_context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        extra_context = extra_context or {}
        User = get_user_model()
        if not self.users_fields:
            self.users_fields = json.dumps(get_simple_fields(User))
        extra_context["field_data"] = self.users_fields
        return extra_context

    def add_view(self, request: HttpRequest, form_url: str = "", extra_context: Any = None) -> HttpResponse:
        return super(SequenceAdmin, self).add_view(
            request,
            form_url=form_url,
            extra_context=self.build_extra_context(extra_context),  # type: ignore
        )

    def change_view(self, request: HttpRequest, object_id: str, form_url="", extra_context: Any = None) -> HttpResponse:
        return super(SequenceAdmin, self).change_view(
            request, object_id, form_url=form_url, extra_context=self.build_extra_context(extra_context)
        )

    def get_urls(self) -> List[URLPattern]:
        urls = super(SequenceAdmin, self).get_urls()
        my_urls = [
            path(
                "<int:sequence_id>/timeline/<int:into_past>/<int:into_future>/",
                self.av(self.timeline),
                name="sequence_timeline",
            ),
        ]

        User = get_user_model()
        if User._meta.get_field("id").get_internal_type() == "UUIDField":
            my_urls += [
                path(
                    "<int:sequence_id>/timeline/<int:into_past>/<int:into_future>/<uuid:user_id>/",  # noqa
                    self.av(self.view_sequence_email),
                    name="view_sequence_email",
                ),
            ]
        else:
            my_urls += [
                path(
                    "<int:sequence_id>/timeline/<int:into_past>/<int:into_future>/<int:user_id>/",  # noqa
                    self.av(self.view_sequence_email),
                    name="view_sequence_email",
                ),
            ]

        return my_urls + urls


admin.site.register(Sequence, SequenceAdmin)


class SentEmailAdmin(admin.ModelAdmin):
    list_display = [f.name for f in SentEmail._meta.fields]
    ordering = ["-id"]


admin.site.register(SentEmail, SentEmailAdmin)


admin.site.register(Campaign, CampaignAdmin)
