...
if TYPE_CHECKING:
    from email_sequences.sequences import SequenceBase
...
from email_sequences.campaigns.models import Campaign
from email_sequences.helpers import parse
from email_sequences.types import BoolOrStr, FExpressionOrStr, FieldValue, TimeDeltaOrStr
...
class AbstractSequence(models.Model):
    ...
    can_resend_sequence = models.BooleanField(
        default=False,
    ...
    unsubscribed_users = models.ManyToManyField(
        ...
        through="UserUnsubscribeSequence",
    ...
    def sequence(self) -> "SequenceBase":
        ...
        # Ignoring this line because we are using AbstractSequence here
        sequence = SequenceBase(
            sequence_model=self,  # type: ignore
            name=self.name,
...
class AbstractSentEmail(models.Model):
    """
    Keeps a record of all sent sequences.
    """
...
class SentEmail(AbstractSentEmail):
    pass
...
class AbstractQuerySetRule(models.Model):
    """
    Allows to apply filters to sequences
    """
...
class UserUnsubscribeSequence(models.Model):
    sequence = models.ForeignKey(
        Sequence,
