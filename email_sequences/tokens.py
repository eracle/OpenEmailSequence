...
class CustomTokenGenerator(PasswordResetTokenGenerator):
    ...
    def check_token(self, user: Optional[AbstractBaseUser], token: Optional[str]) -> bool:
        ...
        # Check that the timestamp/uid has not been tampered with, honouring
        # rotated secrets the same way Django's own generator does.
        return any(
...
class EmailToken:
    """
    A base object for managing token validations and generations for Sequence id and user data.
    """
    ...
    def get_uidb64_token(self, object_id: int) -> Tuple[str, str, str]:
        """
        Returns sequence/campaign and user uidb64 and token for the user, for building url
        """
        sequence_uidb64 = self._get_uidb64(object_id)
        uidb64, token = self.get_uidb64_token_user_only()
        return sequence_uidb64, uidb64, token
    ...
    @classmethod
    def validate_sequence_uidb64(cls, sequence_uidb64: str) -> Optional[Sequence]:
        """
        Validates sequence uidb64 and returns the Sequence object for this string
        """
        try:
            sequence_uid = force_str(urlsafe_base64_decode(sequence_uidb64))
            sequence = Sequence.objects.get(pk=sequence_uid)
        except (TypeError, ValueError, OverflowError, Sequence.DoesNotExist):
            sequence = None
        return sequence
