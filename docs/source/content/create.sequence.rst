...
The Sequence creation page looks like this:

.. image:: ../../images/add_sequence_page.png
  :width: 600
  :alt: Add Sequence page

When you create a sequence, you need at least one queryset so the users in it will be the ones that receive the defined email. In the ``METHOD TYPE`` menu you are able to choose to filter or to exclude the users in the defined queryset.  
...
Here you can relate the Sequence to the corresponding ``Campaign``. Grouping several sequences under a campaign.


Resend sequence field
-----------------

The default behaviour of Sequence library is that an email could only be sent to one user every time.
Now with the field ``Can resend sequence`` you could be able to send the same sequence email to one user multiple times. Every time the user matches the conditions, it will receive the email.
If the field is set to True, the ``SentEmail`` model will not be used to exclude users from queryset. This field could be functional, if you mix it up with some datetime conditions when building your sequence. 
...
Once you configure the key ``SEQUENCE_UNSUBSCRIBE_USERS``to ``True`` in the settings file. The Sequence creation page will change the ``Body html template`` field help text:
...
.. code-block:: python
    ...
    {{unsubscribe_link_sequence}} # Unsubscribe path for the current Sequence
...
Another nice example is the ``SentEmail`` model, it has 8 fields but the sequence field is also a relationship, so it will allow you to filter also over these fields in Sequence model. This will allow you to filter Users that match a specific sequence name sent to that user, using `sent_emails__sequence__name` field.
...
In the previous image, for example, `last_login` is the field in the User model, and `sent_emails__subject` is the subject of the SentEmails model that is related to it. 
...
When you enter a user field that has a date type, Django Sequence Campaigns allows you to enter a date value in natural language combining the current time and some operation with seconds, hours, days, etc. For example, if you have selected the field `last_login` that has a date type, and you want to create a sequence to send emails to the users who logged in exactly one week ago; you can enter:
...
As you can see, the queryset rules creation is very powerful and for each sequence you can add as many as you want.
