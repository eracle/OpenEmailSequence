...
from email_sequences.models import Sequence, QuerySetRule
...
class TestCustomMessages:
    ...
    @classmethod
    def setup_class(cls):
        cls.old_msg_classes = getattr(settings, "SEQUENCE_MESSAGE_CLASSES", None)
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            (
                {"plain": "tests.PlainSequenceEmail"},
                None,
                1,
                1,
            ...
            (
                {
                    "plain": "tests.test_custom_messages.PlainSequenceEmail",
                },
                "plain",
                1,
                1,
            ...
            (
                {
                    "default": "tests.test_custom_messages.PlainSequenceEmail",
                },
                None,
                1,
                1,
    ...
    def test_emails(
        ...
        if settings_message_config:
            settings.SEQUENCE_MESSAGE_CLASSES = settings_message_config
        if message_class:
            self.model_sequence.message_class = message_class
            self.model_sequence.save()
        result = self.model_sequence.sequence.send()
